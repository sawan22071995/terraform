...
# K8S Deployment
#####################################################################
resource "kubernetes_deployment" "omnistore-admin-services" {
  metadata {
    name = "omnistore-admin-services"
    namespace = var.namespace
    labels = {
      app = "omnistore-admin-services"
    }
  }
  spec {
    ...
    template {
      ...
      spec {
        container {
          image = "gcr.io/omnistore-admin-services-kubernetes/omnistore-admin-services-example-paycheck:1.0"
          name = "omnistore-admin-services"
